...
def calculate_fid_given_paths(paths0, paths1, batch_size, cuda, dims):
    ...
    model = vgg_face_dag('/home/lingna/workspaces/vggface/vgg_face_dag.pth').cuda()
...
if __name__ == '__main__':
    ...
    # fid_value = calculate_fid_given_paths(path1, path2, 8, 1, 4096)
    # print(fid_value)

    path2 = '/home/lingna/dataset/cuhk_a'
    paths_list = os.listdir('./output')
    paths_list.sort()
    for name in paths_list:
        path1 = os.path.join('./output', name)
        fid_value = calculate_fid_given_paths(path1, path2, 8, 1, 4096)
        print('name = %s: %.5f'%(name, fid_value))
'''
if __name__ == '__main__':
