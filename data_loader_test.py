...
def make_dataset1(dir, file_p, file_s):
    imgA = []
    imgB = []

    p_dir = os.listdir(os.path.join(dir, file_p))
    p_dir.sort()
    s_dir = os.listdir(os.path.join(dir, file_s))
    s_dir.sort()
    for (p, s) in zip(p_dir, s_dir):
        imgA.append(os.path.join(dir, file_p, p))
        imgB.append(os.path.join(dir, file_s, s))
    return imgA, imgB
...
class MyDataset(data.Dataset):
    def __init__(self, opt, isTrain=0, transform=None, return_paths=None, loader=default_loader):
        ...
        imgs_test = make_dataset1(self.opt.dataroot, 'cuhk_b', 'cuhk_a')
        # if len(imgs) == 0:
        #     raise (RuntimeError("Found 0 images in: " + self.opt.dataroot + dir + "\n" "Supported image extensions are: " + ",".join(IMG_EXTEND)))
    ...
    def __getitem__(self, index):
        ...
        #     imgB = self.process_img(imgB, i, j, padding)
            ...
            path_A = self.imgs_test[0][index]
            path_B = self.imgs_test[1][index]
            label_A = path_A.split('/')[-1].split('.')[0]
            label_B = path_B.split('/')[-1].split('.')[0]
        ...
        return imgA, imgB, label_A, label_B

    def __len__(self):
        if self.isTrain == 0:
            return len(self.imgs[1])
        else:
            return len(self.imgs_test[1])
